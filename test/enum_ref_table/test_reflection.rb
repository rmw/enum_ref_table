...
describe EnumRefTable::Reflection do
  let(:reflection) { EnumRefTable::Reflection.new(:gender, female: 1, male: 2) }

  describe "#initialize" do
    it "uses symbol values if :type is :symbol" do
      reflection = EnumRefTable::Reflection.new(:gender, type: :symbol)
      reflection.add_value 1, 'female'
      reflection.value(1).must_equal(:female)
    end

    it "uses string values if :type is :string" do
      reflection = EnumRefTable::Reflection.new(:gender, type: :string)
      reflection.add_value 1, :female
      reflection.value(1).must_equal('female')
    end

    it "raises an ArgumentError if :type is something else" do
      ->{ EnumRefTable::Reflection.new :gender, type: :other }.must_raise ArgumentError, /invalid type/
    end
  end

  describe "#initialize_copy" do
    it "sets the same name" do
      reflection = EnumRefTable::Reflection.new(:gender)
      copy = reflection.dup
    ...
    it "sets the same id_name" do
      reflection = EnumRefTable::Reflection.new(:gender, id_name: :status_id)
      copy = reflection.dup
  ...
  describe '#id_name' do
    it "can be set with an :id_name option" do
      reflection = EnumRefTable::Reflection.new(:gender, id_name: :gender_number)
