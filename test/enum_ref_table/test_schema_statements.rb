...
describe EnumRefTable::SchemaStatements do
  use_database

  describe "#create_enum_ref_table" do
    it "creates an enum table with the default name and default schema if none are given" do
      connection.create_enum_ref_table :genders
      connection.tables.sort.must_equal ['enum_ref_tables', 'genders']
    ...
    it "accepts :values as a Hash" do
      connection.create_enum_ref_table :genders, values: {female: 2, male: 5}
      read_table('genders').must_equal [[2, 'female'], [5, 'male']]
    end

    it "accepts :values as an Array" do
      connection.create_enum_ref_table :genders, values: [:female, :male]
    ...
    it "passes on other options to create_table" do
      ...
      ->{ connection.create_enum_ref_table :genders }.must_raise(ActiveRecord::StatementInvalid)
      connection.create_enum_ref_table :genders, force: true
    end

    it "allows configuring the type column in the block" do
      connection.create_enum_ref_table :genders do |t|
    ...
    it "records the enum_ref_table" do
      connection.create_enum_ref_table :genders
      result = connection.execute "SELECT table_name FROM enum_ref_tables"
    ...
    it "performs the necessary SQL escaping in value names" do
      connection.create_enum_ref_table "a'b" do |t|
        t.add "c'd", 1
      end
  ...
  describe "#change_enum_ref_table" do
    before do
    ...
    it "inserts values added in the block" do
      connection.change_enum_ref_table :genders do |t|
        t.add :other, 3
      end
    ...
    it "performs the necessary SQL escaping in value names" do
      ...
      connection.change_enum_ref_table "a'b" do |t|
        t.remove "c'd"
        t.add "e'f", 1
      end
  ...
  describe "#drop_enum_ref_table" do
    it "removes the table from enum_ref_tables if required" do
      connection.create_enum_ref_table :genders
      connection.drop_enum_ref_table :genders
      result = connection.execute "SELECT table_name FROM enum_ref_tables"
      result.to_a.must_equal []
    end
  end

  describe "#enum_ref_tables" do
    it "returns the names of the enum tables" do
      connection.create_enum_ref_table :genders
      connection.create_enum_ref_table :statuses
      connection.enum_ref_tables.must_equal ['genders', 'statuses']
    end

    it "returns no tables if no enum tables have ever existed" do
      connection.enum_ref_tables.must_equal []
    end
    ...
    it "clears memoized tables when an enum table is created" do
      connection.create_enum_ref_table :genders
      connection.enum_ref_tables.must_equal ['genders']
      connection.create_enum_ref_table :statuses
      connection.enum_ref_tables.must_equal ['genders', 'statuses']
    end
    ...
    it "clears memoized tables when an enum table is dropped" do
      ...
      connection.enum_ref_tables.must_equal ['genders', 'statuses']
      connection.drop_enum_ref_table :genders
      connection.enum_ref_tables.must_equal ['statuses']
    end
  end
